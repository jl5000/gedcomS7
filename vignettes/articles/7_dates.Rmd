...
Calendar dates are the most common type of date used. They can not only be used to create specific dates like `DateExact()`, they underpin the other date types described below. They can be created using `DateCalendar()`:
...
The Gregorian calendar is implicitly assumed by default. The Julian calendar can be specified by setting `julian = TRUE`:

```{r}
DateCalendar(year = 1999, month = 4, julian = TRUE)
```

No other calendars apart from the Gregorian and Julian calendars are supported by the package.

## Approximate dates

Approximate dates (i.e. those expressing uncertainty) use the qualifiers 'about', 'calculated', or 'estimated' in conjunction with a `DateCalendar()` object:
...
Date periods can take one or two `DateCalendar()` objects:
...
Providing only one date gives a semi-infinite period. If a mix of calendars are used with a finite period, they are made explicit, e.g.
