...
The goal of `gedcomS7` is to handle genealogical data as GEDCOM files. This package succeeds the `tidyged` package and the other packages in the `gedcompendium`. The key changes are:
...
The package does not currently implement the following:

* LDS_SPOUSE_SEALING structures of Family records 
* LDS_INDIVIDUAL_ORDINANCE of Individual records
* Extension tags

If you import a file with any of these features, the package may behave unpredictably.
...
ged <- read_gedcom(system.file("extdata", "555SAMPLE.GED", package = "gedcomS7"))
ged

ged <- new_gedcomS7(my_name = "Jamie")
ged
```
